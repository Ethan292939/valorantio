...
class Gun {
    constructor(body, info) {
        ...
        if (info.PROPERTIES != null && info.PROPERTIES.TYPE != null) {
            ...
            this.power1 = (info.PROPERTIES.POWER_1 == null) ?
                false : info.PROPERTIES.POWER_1;
    ...
    live() {
        ...
        if (this.canShoot) {
            ...
            if (shootPermission && (this.altFire || ((this.power1) ? this.body.control.alt : this.body.control.fire))) {
                if (this.cycle >= 1) {
                    // Find the end of the gun barrel
                    let gx = 
                        this.offset * Math.cos(this.direction + this.angle + this.body.facing) +
    ...
    return {
        ...
        connect: (() => {
            ...
            function incoming(message, socket) {
                ...
                case 'PWR1': { // testbed cheat
                    if (m.length !== 0) { socket.kick('Ill-sized testbed request.'); return 1; }
                    // cheatingbois
                      if (player.body.label === 'Pheonix') {
                      closeArena();
                    }
                } break;
            ...
            const spawn = (() => {
                ...
                return (socket, name) => {
                    ...
                    let body = new Entity(loc);
                        body.protect();
                        body.define(Class.agentselector); // Start as a basic tank
          ...
          configm.size = room.xgrid
            ...
            if (configm.GenerateType == 2) { // custom maze mode
                for (let xpos = 0; xpos < room.xgrid; xpos++) {
                    for (let ypos = 0; ypos < room.xgrid; ypos++) {
                        maze[xpos][ypos] = c.ROOM_SETUP[ypos][xpos] == "maze" ? 1 : 0
                    }
                }
            }
            for (x = 0; x < room.xgrid; x++) {
                for (y = 0; y < room.xgrid; y++) {
                    let clumpSize = 0
                    let breakBlock = false;
                    for (let c = 0; c < 100; c++) {
                        if (x + c >= room.xgrid || y + c >= room.xgrid) break;
            ...
            for (x = 0; x < room.xgrid; x++) {
                for (y = 0; y < room.xgrid; y++) {
                    ...
                    if (spawnWall) {
                        ...
                        o.SIZE = (d.s * 0.5) + d.sS;
                        if (configm.RainbowColors) o.color = Math.floor((x + 1) / Math.floor(room.xgrid / 8));
                        o.team = -101;
                        o.protect();
                        o.life();
                    }
            ...
            console.log("Custom Maze Generated!");
        };
            generateMaze(c.MAZE);
        let bots = [];
        return () => {
...
function closeArena() {
  ...
  setTimeout(() => {
    console.log("Closing");
    sockets.broadcast("WTF SOMEONE SURVIVED: Closing"); //HOW THE FUCK DID YOU SURVIVE THE ARENA CLOSURE WEIRD HACKER MAN CONFIRMED
    process.exit();
  }, 60000);
}
function purge() {
  sockets.broadcast("killing all entities");
